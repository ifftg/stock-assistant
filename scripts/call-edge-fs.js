...
async function main(){
  const body = { action: 'fetch_stock_list', page_size: 50, page: 1, fs: 'm:1+t:3,m:0+t:7' };
  const res = await fetch(url, { method:'POST', headers: { 'Content-Type':'application/json', apikey: anon, Authorization: `Bearer ${anon}` }, body: JSON.stringify(body) });
